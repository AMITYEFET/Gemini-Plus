let currentSettings = {
    ...
    hideDisclaimer: false, uiLanguage: 'device'
};
...
// פונקציית כפיית שפה דרך שורת הכתובת (URL)
function enforceLanguage(data) {
    if (data.uiLanguage) {
        let targetLang = data.uiLanguage;
        if (targetLang === 'device') {
            targetLang = navigator.language.split('-')[0]; // חילוץ השפה הכללית, למשל "he" מ-"he-IL"
        }
        
        const url = new URL(window.location.href);
        const currentHl = url.searchParams.get('hl');
        
        if (currentHl !== targetLang) {
            url.searchParams.set('hl', targetLang);
            window.location.replace(url.toString()); // ריענון מיידי עם השפה החדשה
            return true; 
        }
    }
    return false;
...
function updateUI(data) {
    // אם בוצע ריענון בגלל החלפת שפה, אנחנו עוצרים פה כדי לא להעמיס על הדפדפן
    if (enforceLanguage(data)) return;
...
function injectGeminiTools() {
    ...
    const showDownload = currentSettings.downloadEnabled;
    ...
    for (let m of mutations) {
        for (let node of m.addedNodes) {
            if (node.nodeType === 1) {
                ...
                if (menu) {
                    if (!lastMenuTriggerElement) continue;

                    const isCodeContext = lastMenuTriggerElement.closest('pre, code, .monaco-editor, .xap-monaco-container, [class*="workspace"], snippet-action-bar, .snippet-action-bar');
                    const isMessageAction = lastMenuTriggerElement.closest('.message-actions, response-action-bar, [data-test-id="response-action-bar"]');
                    
                    if (!isCodeContext || isMessageAction) {
                        continue; 
                    }
